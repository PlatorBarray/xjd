...
{	
    ...
    $count = goods_count_by_brand($brand_id, $cate,$sort);
 ...
 * @param   integer     $cate
 * @param   integer     $have_goods   是否只显示有货
 * @return  integer
 */
function goods_count_by_brand($brand_id, $cate = 0, $sort = '')
{
	global $suppId;
...
function brand_get_goods($brand_id, $cate, $size, $page, $sort, $order)
{
	...
	if($sort ==goods_number)
	{
		$cate_where .= " AND g.goods_number != 0 ";
	}
    /* 获得商品列表 */
