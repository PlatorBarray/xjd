...
$smarty->assign('getuserinfo', $GLOBALS['db']->getRow($sql));
$back_act = '';
...
{
	$get_username=$_GET['uname'];
	if(!empty($get_username))
	{
		$himgsql="select headimg from ".$GLOBALS['ecs']->table('users')." where user_name='".$get_username."' or email='".$get_username."' or mobile_phone='".$get_username."'";
		$himg=$GLOBALS['db']->getRow($himgsql);
		header('Content-type: text/json');
		if(!empty($himg))
		{
			$result=array('status'=>1,'img_url'=>$himg['headimg']);
			echo json_encode($result);
			exit();
		}else
		{
			$result=array('status'=>0,'img_url'=>'/themes/68ecshopcom_360buy/images/people.gif');
			echo json_encode($result);
			exit();
		}
	}else
