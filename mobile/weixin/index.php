...
if ($event['event'] == "subscribe") { //用户关注
	//old
	// $msg = $weixinconfig['followmsg'];
	// if(intval($followInfo['ecuid']) == 0){
	// 	$msg .= "<a href='{$weburl}mobile/user.php?wxid={$wxid}'>点击绑定</a>";
	// 	if($data['Ticket'])
	// 	{
	// 		$sql = "SELECT * FROM " . $GLOBALS['ecs']->table('weixin_qcode') . 
	// 				" WHERE qcode = '" . $data['Ticket'] . "'";
	...
	// 			 $newsData[0]['Url'] = $baseurl."mobile/supplier.php?suppId=" . $code['content'];
	...
	// 			{
	// 				 $api->bind_record($wxid,$code['content']);
	// 			}
	// 		}
	// 	}
	// }
	// echo $weixin->text($msg)->reply();//发送欢迎信息
	// exit;
...
if ($event['event'] == "unsubscribe"){ // 用户主动删除
	...
	//$api->unBindUser($wxid);	//周辉解绑会员(一般情况下,取消关注是不解绑会员的.除非用户主动点击解绑.)
	exit;
}
//查询用户输入是否为指定命令
if($type == "text"){
