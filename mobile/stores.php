...
{ 
	...
	$shop_list = get_all_supplier();
        //获取评分
    foreach($shop_list['shops'] as $k=>$v){
            ...
            $shop_list['shops'][$k]['comment_rand'] = $avg_comment;
            $shop_list['shops'][$k]['server'] = $avg_server;
            $shop_list['shops'][$k]['shipping'] = $avg_shipping;
            $shop_list['shops'][$k]['pingfen'] = round((($avg_comment+$avg_server+$avg_shipping)/3),0);
    }    

    //获取店铺logo
    foreach($shop_list['shops'] as $k=>$v){
        $sql = "select value from ". $GLOBALS['ecs']->table('supplier_shop_config') ." where code = 'shop_logo' and supplier_id = ".$v['supplier_id'];
        $shop_list['shops'][$k]['shop_logo'] = $GLOBALS['db']->getOne($sql);
    }
	//推荐分类中的店铺
...
//    $smarty->assign('record_count', $shop_list['record_count']);
    $smarty->assign('pager',   $shop_list['pager']);
...
function get_all_supplier(){
	...
	foreach($arr as $key=>$val){
		...
		$arr[$key]['address'] = trim($arr[$key]['address'],',');
                $arr[$key]['is_guanzhu'] = is_guanzhu($val['supplier_id']);
		//店铺中有多少商品
		$goodsInfo = get_street_goods_info($val['supplier_id']);
		$arr[$key]['goods_number'] = $goodsInfo['num'];
		$arr[$key]['goods_info'] = $goodsInfo['info'];
                //添加联系电话
                $arr[$key]['contacts_phone'] = $GLOBALS['db']->getOne("select contacts_phone from ".$GLOBALS['ecs']->table('supplier')." where supplier_id=".$val['supplier_id']);
                $arr[$key]['service_phone'] = $GLOBALS['db']->getOne("select value from ".$GLOBALS['ecs']->table('supplier_shop_config')." where code='service_phone' AND supplier_id=".$val['supplier_id']);
	}
        $pager = get_pager('stores.php', array('act' => ''),  $filter['record_count'], $filter['page'], $filter['page_size'] );
    return array('shops' => $arr, 'filter' => $filter, 'pager' => $pager);
}
...
function get_street_goods_info($suppid){
	...
	if($allnum > 0){
		...
		foreach($goodsInfo as $key=>$row){
			...
			$goodsInfo[$key]['goods_thumb']      = get_pc_url().'/'.get_image_path($row['goods_id'], $row['goods_thumb'], true);
