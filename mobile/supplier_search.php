...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    $pager = get_pager('supplier.php', array('go' => 'search','suppId'=>$_GET['suppId'],'keywords'=>$keywords), $count, $page, $size);
...
function category_get_goods($children, $size, $page,$keywords='')
{
    ...
    if(empty($keywords)){
    	...
    	$sql = 'SELECT DISTINCT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, exclusive, ' .
