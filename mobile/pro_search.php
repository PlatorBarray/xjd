...
if ($_REQUEST['act'] == 'ajax_list'){
    ...
    $goodslist = get_pro_search_list( $limit );
    foreach($goodslist as $key=>$val){
            $GLOBALS['smarty']->assign('goods',$val);
            $GLOBALS['smarty']->assign('key',$key);
            $res[]['info']  = $GLOBALS['smarty']->fetch('library/pro_search_goods.lbi');
    }
    die($json->encode($res));
}
...
function get_pro_search_list($limit){
    ...
    /* 查询商品 */
    $sql = "SELECT g.goods_id, g.goods_name, g.market_price, g.is_new, g.is_best, g.is_hot,g.goods_number, g.shop_price AS org_price, ".
