...
elseif ($_REQUEST['act'] == 'update')
{
	...
	elseif($parent_id > 0)
	{
		$sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id = '$parent_id'";
		if($db->getOne($sql) == 0)
		{
			sys_msg('该分销商不存在，请重新输入！',0,$link); 
		}
		else
		{
			 ...
			 if($uid == $parent_id)
			 {
				 sys_msg('上级分销商不能绑定自己！',0,$link); 
			 }
			 else
			 {
				 $sql = "SELECT parent_id FROM " . $ecs->table('users') . " WHERE user_id = '$parent_id'";
				 $pid = $db->getOne($sql);
				 if($pid == $uid)
				 {
					  sys_msg('不能绑定自己的下级分销商！',0,$link);
				 }
				 else
				 {
	...
	$sql = "update ".$ecs->table('users')." set `real_name`='$real_name',`card`='$card',`country`='$country',`province`='$province',`city`='$city',`district`='$district',`address`='$address',`status`='$status' ".$con." where user_name = '".$username."'";
	$db->query($sql);
    ...
    $links[0]['href']    = 'distributor.php?act=list&' . list_link_postfix();
...
elseif ($_REQUEST['act'] == 'batch_remove')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        $lnk[] = array('text' => $_LANG['go_back'], 'href'=>'distributor.php?act=list');
...
elseif ($_REQUEST['act'] == 'aff_list')
{
    ...
    $smarty->assign('record_count', $user_list['record_count']);
    $smarty->assign('full_page',    1);
    $smarty->assign('action_link',  array('text' => $_LANG['back_note'], 'href'=>"distributor.php?act=edit&id=$auid"));
...
function user_list()
{
    ...
    if ($result === false)
    {
		...
		$filter['sign'] = empty($_REQUEST['sign']) ? '' : trim($_REQUEST['sign']);
