...
function assign_template_supplier($ctype = '', $catlist = array())
{
    ...
    $smarty->assign('activitynum',      get_supplier_activity_count());//商家优惠活动数量
    ...
    $smarty->assign('fensi',get_supplier_fensi_count()); //获取店铺被搜藏数量
...
function get_supplier_fensi_count($suppid=0){
    $suppid = (intval($suppid)>0) ? intval($suppid) : intval($_GET['suppId']);
    return $GLOBALS['db']->getOne("SELECT count(*) FROM " .$GLOBALS['ecs']->table('supplier_guanzhu') ." WHERE supplierid=$suppid");
}
/**
 * 获取店铺优惠活动数量
 */
function get_supplier_activity_count($suppid=0){
    $suppid = (intval($suppid)>0) ? intval($suppid) : intval($_GET['suppId']);
    $nowtime = gmtime();
    $sql = "SELECT count(*) FROM ".$GLOBALS['ecs']->table('favourable_activity') ." WHERE supplier_id = $suppid AND start_time<=".$nowtime." AND end_time>=".$nowtime;
    $activitynum = $GLOBALS['db']->getOne($sql);
    return $activitynum;
}
