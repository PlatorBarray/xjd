...
function edit_profile($profile)
{
    ...
    if(!empty($profile['user_name'])){
        $user_id_exist = $GLOBALS['db']->getOne("select user_id from ". $GLOBALS['ecs']->table('users') ." where user_id <> '".$profile['user_id']."' AND user_name = '".$profile['user_name']."'");
        if($user_id_exist){
            $GLOBALS['err']->add($GLOBALS['_LANG']['user_name_exist']);
            return false;
        }
    }
    $cfg = array();
...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        else if ($row['order_status'] == OS_SPLITED)
        {
            ...
            {
                @$row['handler'] = '<span style="color:#e41735">'.$GLOBALS['_LANG']['ss_received'] .'</span>';
            }
            else
            {
        ...
        {
            $row['handler'] = '<span style="color:#e41735">'.$GLOBALS['_LANG']['os'][$row['order_status']] .'</span>';
        }
 ...
 * @return type
 */
function get_user_orders_ajax($user_id, $limit,$where='')
{
    /* 取得订单列表 */
    $arr    = array();
    ...
    foreach ($res as $key=>$row)
    {
        ...
        {
            $row['handler'] = '<a>'.$GLOBALS['_LANG']['os'][$row['order_status']] .'</a>';
        }
	...
	$row['order_status1'] = $row['order_status'];
		...
		$arr[$row['order_id']] = array('order_id'       => $row['order_id'],
						...
						'weixiu_time'    => $row['weixiu_time'],
                                                ...
                                                'customers' => is_customers(CUSTOMER_SERVICE, $row['supplier_id']),
                                                'tel' => get_tel($row['supplier_id']),
...
function get_order_goods($order)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $row['goods_thumb'] = get_pc_url().'/'.get_image_path($row['goods_id'], $row['goods_thumb'],true);
...
function get_order_detail($order_id, $user_id = 0)
{
    ...
    if (!empty($order['invoice_no']))
    {
         ...
         if (file_exists($plugin))
        {
	      ...
	      $order['invoice_no'] = '<a href="http://m.kuaidi100.com/index_all.html?type='.$order['shipping_name'].'&postid='.$order['invoice_no'].'&callbackurl=javascript:history.back(-1)">'.$order['invoice_no'].'</a>';
        }
    }
        ...
        $order['order_status'] == OS_CONFIRMED))
    {
        ...
        else
        {
            ...
            /* 修改支付按钮 针对微信支付 支付宝*/
            if($order['pay_name'] == '支付宝'){
                $order['pay_online'] = '<div style="text-align:center"><input type="button" onclick="window.location.href=\'./pay/alipayapi.php?out_trade_no='.$order['log_id'].'&total_fee='.$order['order_amount'].'\'"/></div>';
            }
            if($order['pay_name'] == '微信支付'){
                $order['pay_online'] = '<div style="text-align:center"><input type="button" onclick="window.location.href=\'./weixinpay.php?out_trade_no='.$order['log_id'].'\'"/></div>';
            }
        }
    }
    else
...
{
    $sql = "SELECT u.bonus_sn, u.order_id, u.supplier_id, b.type_name, b.type_money, b.min_goods_amount, b.use_start_date, b.use_end_date ".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $row['supplier_name'] = $row['supplier_id']>0? $GLOBALS['db']->getOne("select value from ". $GLOBALS['ecs']->table('supplier_shop_config')." where code = 'shop_name' and supplier_id = '".$row['supplier_id']."'"):'网站自营';
        $arr[] = $row;
    }
    return $arr;
 ...
 * @param type $order
 */
function get_order_handler($order){
    if ($order['order_status'] == OS_UNCONFIRMED)
        {
        if(in_array($order['pay_id'],payment_id_list(true))){
                $handler['handler'] = '<span>'.$GLOBALS['_LANG']['cod'].'</span>';
            }
            $handler['handler'] .= " <a href=\"user.php?act=cancel_order&order_id=" .$order['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_cancel']."')) return false;\">".$GLOBALS['_LANG']['cancel']."</a>";
        }
    else if ($order['order_status'] == OS_SPLITED || $order['order_status'] == OS_CONFIRMED)
        {
            /* 对配送状态的处理 */
            if ($order['shipping_status'] == SS_SHIPPED)
            {
				$back_num = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('back_order') . " WHERE order_id = " . $order['order_id'] . " AND status_back < 6 AND status_back != 3");
				if ($back_num > 0)
				{
					$back_info = "此单存在正在退货商品，确认收货退货申请将取消。";
				}
...
//				@$row_time = $okgoods_time - (local_date('d',gmtime()) - local_date('d',$row['shipping_time']));
            ...
            else if ($order['shipping_status'] == SS_RECEIVED)
            {
                ...
                $handler['handler'] .= " <a href='kuaidi_list.php?order_id=$order[order_id]'>".$GLOBALS['_LANG']['view_wuliu'] ."</a>";
                $handler['handler'] .= ' <span>'.$GLOBALS['_LANG']['ss_received'] .'</span>';
            }
        }
        else
        {
...
function get_tel($supplier_id = 0){
    if($supplier_id>0){
        $sql = "select value from ".$GLOBALS['ecs']->table('supplier_shop_config')." where code = 'service_phone' and supplier_id = $supplier_id and parent_id = 1";
    }else{
