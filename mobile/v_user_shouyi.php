...
if (!$smarty->is_cached('v_user_shouyi.dwt', $cache_id))
{
	...
	$smarty->assign('yes_distrib_count',get_count_distrib_order_by_user_id($_SESSION['user_id'],1)); //已分成订单数量
	$smarty->assign('no_distrib_count',get_count_distrib_order_by_user_id($_SESSION['user_id'],0));	//未分成订单数量
	$smarty->assign('cancel_distrib_count',get_count_distrib_order_by_user_id($_SESSION['user_id'],2));	//撤销分成订单数量
