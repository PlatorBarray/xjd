...
if($act == 'list')
{
    ...
    _wap_assign_header_info('订单列表','',0,0,0,0);
...
elseif($act == 'edit')
{
    ...
    if ($order_id > 0)
    {
        ...
        {
            if ($step == 'shipping' || $step == 'payment' || $step == 'money' || $step == 'consignee')
            {
                sys_msg($_LANG['cannot_edit_order_shipped'],1);
            }
            else
            {
                $step = 'invoice';
...
elseif($act == 'delivery_info')
{
    ...
    $smarty->assign('action_act', ($delivery_order['status'] == 2) ? 'delivery_ship' : 'cancel_delivery');
    assign_query_info();
...
elseif ($_REQUEST['act'] == 'operate')
{
    ...
    elseif (isset($_POST['ship']))
    {
        ...
        assign_query_info();
        _wap_display_page('order_delivery_info.htm');
        exit;
    }
    /* 未发货 */
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        $where = "WHERE o.supplier_id='".$_SESSION['supplier_id']."' AND extension_code <> 'virtual_good' ";
    ...
    {
                /* 取得订单商品 */
        $_goods = get_order_goods(array('order_id' => $value['order_id'], 'order_sn' => $value['order_sn']));
        $row[$key]['goods_list']  = $_goods['goods_list'];
...
function delivery($order_id,$delivery_id,$express_no)
{
     /* 定义当前时间 */
    define('GMTIME_UTC', gmtime()); // 获取 UTC 时间戳
    $ecs = $GLOBALS['ecs'];
...
function get_order_goods($order)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $row['goods_thumb'] = get_pc_url().'/'.get_image_path($row['goods_id'], $row['goods_thumb']);
        $goods_list[] = $row;
    }
