...
function order_info($order_id, $order_sn = '')
{
    ...
    if ($order_id > 0)
    {
        $sql = "SELECT *, " . $total_fee . " FROM " . $GLOBALS['ecs']->table('order_info') .
                " WHERE order_id = '$order_id' AND supplier_id = ".$_SESSION['supplier_id'];
    }
    else
    {
        $sql = "SELECT *, " . $total_fee . "  FROM " . $GLOBALS['ecs']->table('order_info') .
                " WHERE order_sn = '$order_sn' AND supplier_id = ".$_SESSION['supplier_id'];
    }
    ...
    if ($order)
    {
        ...
        $order['formatted_pay_time']       = local_date($GLOBALS['_CFG']['time_format'], $order['pay_time']);
        $order['formatted_shipping_time']       = local_date($GLOBALS['_CFG']['time_format'], $order['shipping_time']);
    }
...
function get_total_bonus($supplier_money_info)
{
	...
	$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . 

SESS_ID . "' AND c.user_id=0 ";
    

	$sql_where1 = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . 

SESS_ID . "' AND user_id=0 ";
