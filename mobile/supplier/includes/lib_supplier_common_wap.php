...
function _wap_finished(){
    global $db,$ecs;
    $finished = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('order_info') .' WHERE 1 '. order_query_sql('finished').' AND supplier_id='.$_SESSION['supplier_id']);
    return $finished;
}
...
function _wap_await_receipt(){
    global $db,$ecs;
    $await_receipt = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('order_info') .' WHERE 1 '. order_query_sql('await_receipt').' AND supplier_id='.$_SESSION['supplier_id']);
...
function _wap_assign_header_info($ur_here='',$page_title='',$no_back=0,$no_refresh=0,$logout_button=0,$header_search=0)
{
    global $smarty;
	...
	$smarty->assign('header_search',$header_search);
}
