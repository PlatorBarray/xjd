...
function assign_status_list($sel_status)
{
	global $smarty;
	$col_size = 4;
	$status_list = $GLOBALS['_LANG']['cs'];
	$keys = array_keys($status_list);
	$status_list_new = array();
	$sel_scr = 1;
	for($i = 0;$i < count($status_list);$i++)
	{
		$cur_scr = ceil(($i+1)/$col_size);
		if($keys[$i] == $sel_status)
		{
			$sel_scr = $cur_scr;
		};
		$status_list_new[$cur_scr][$keys[$i]] = $status_list[$keys[$i]];
	}
	
	$status_list = $status_list_new;
	$smarty->assign('sel_status_scr',$sel_scr);
	$smarty->assign('status_scr_count',count($status_list));
