...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'ajax')
{
	...
	$amount = isset($_POST['amount']) ? intval($_POST['amount']) : $size;
	...
	else
	{
		...
		if($cat_ids)
		{
			...
			for($i = 0; $i < count($ids); $i++)
			{
				if($i == 0)
				{
					 $children = get_children($ids[$i]);
				} 
				else
				{
					 $children .= " OR " . get_children($ids[$i]);
				}
			}
		}
	}
...
elseif(!empty($_REQUEST['act']) && $_REQUEST['act'] == 'ajax_change_filter')
{
    include('includes/cls_json.php');
    $json   = new JSON;
    $last = empty($_POST['last'])?'0':intval($_POST['last']);
	$amount = empty($_POST['amount'])?$size:intval($_POST['amount']);
	$limit = 'LIMIT '.$last.','.$amount;
	if($cat_id)
	{
    	$children = get_children($cat_id);
    ...
    $goodslist = category_get_goods($children, $brand, $price_min, $price_max, $ext, $size, $page, $sort, $order, $limit);
    foreach($goodslist as $val)
    {
       $smarty->assign('goods', $val);
       $res[]['info']  = $GLOBALS['smarty']->fetch('library/shop_list_ajax.lbi');
...
if (!$smarty->is_cached('v_shop_list.dwt', $cache_id))
{
        ...
        */
        ...
        foreach ($price_grade as $key=>$val)
        {
            ...
            if (isset($_REQUEST['price_min']) && $price_grade[$temp_key]['start'] == $price_min && $price_grade[$temp_key]['end'] == $price_max)
            {
                ...
                $smarty->assign('selected_price','true');
                $smarty->assign('selected_price_range',$price_grade[$temp_key]['formated_start'].'-'.$price_grade[$temp_key]['formated_end']);
	    }
            else
            {
        ...
        $price_grade[0]['formated_start1'] = '0';
        $price_grade[0]['formated_end1'] = '0';
	...
	if($children)
	{
		$con = " AND ($children OR " . 'gc.cat_id ' . db_create_in(array_unique(array_merge(array($cat_id), array_keys(cat_list($cat_id, 0, false))))) . ")";
	}
    ...
    $sql = "SELECT b.brand_id, b.brand_name, COUNT(*) AS goods_num ".
            ...
            "WHERE g.brand_id = b.brand_id $con  AND b.is_show = 1 " .
    ...
    foreach ($brands AS $key => $val)
    {
        ...
        if ($brand == $brands[$temp_key]['brand_id'])
        {
            ...
            $smarty->assign('selected_brand','true');
            $smarty->assign('selected_brand_name',$val['brand_name']);
        }
        else
        {
    ...
    assign_pager('v_shop_list',            $cat_id, $count, $size, $sort, $order, $page, '', $brand, $price_min, $price_max, $display, $filter_attr_str); // 分页
    assign_dynamic('v_shop_list'); // 动态内容
}
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order, $limit='')
{
    ...
    $sql = "SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, " .
                ...
                "g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img FROM " . $GLOBALS['ecs']->table('ecsmart_distrib_goods') . " as dg left join " . 
	$GLOBALS['ecs']->table('goods') . " as g on dg.goods_id = g.goods_id ".
            "WHERE $where AND (dg.distrib_time = 0 OR (dg.start_time <='" . gmtime() . "' AND dg.end_time >= '" . gmtime() . "'))  ORDER BY $sort $order $limit";
    if ($sort=='salenum')
	{
		 $sql = "SELECT SUM(o.goods_number) as salenum,g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, " .
                ...
                "g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img FROM " . $GLOBALS['ecs']->table('ecsmart_distrib_goods') . " as dg left join " . 
	$GLOBALS['ecs']->table('goods') . " as g on dg.goods_id = g.goods_id ".
	"LEFT JOIN " . $GLOBALS['ecs']->table('order_goods') .  " as o ON o.goods_id = dg.goods_id " . 
            "WHERE $where AND (dg.distrib_time = 0 OR (dg.start_time <='" . gmtime() . "' AND dg.end_time >= '" . gmtime() . "')) group by o.goods_id $limit";
...
function get_cagtegory_goods_count($children, $brand = 0, $min = 0, $max = 0, $ext='')
{
	...
	/* 返回商品总数 */
    return $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('ecsmart_distrib_goods') . " AS d," . $GLOBALS['ecs']->table('goods') . " as g WHERE d.goods_id = g.goods_id $where $ext");
}
...
function get_child($cat_id)
{
	 if($cat_id > 0)
	 {
	 	return 'g.cat_id ' . db_create_in($cat_id);
	 }
}

function get_catids($user_id)
{
