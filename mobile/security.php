...
function action_check_mobile_exist()
{
	...
	if($user->check_mobile_phone($email))
	{
		echo 'false';
	}
	else
...
function action_validate () {
	...
	{
		exit(json_encode(array('error' => 1, 'content' => '验证类型不能为空', 'url' => 'security.php')));
	}
	...
	if($validate_type == 'email')
	{
		...
		if($result == 1)
		{
			exit(json_encode(array('error' => 1, 'content' => $_LANG['msg_email_blank'], 'url' => '')));
		}
		else if($result == 2)
		{
			exit(json_encode(array('error' => 1, 'content' => $_LANG['msg_email_format'], 'url' => '')));
		}
		else if($result == 3)
		{
	...
	else if($validate_type == 'password')
	{
		...
		{
			exit(json_encode(array('error' => 1, 'content' => '登录密码错误', 'url' => '')));
		}
	}
	else
	{
		/* 无效的注册类型 */
		//show_message('非法验证参数', $_LANG['sign_up'], 'user.php?act=register', 'error');
		exit(json_encode(array('error' => 1, 'content' => '非法验证参数', 'url' => '')));
	}
...
function action_do_password_reset ()
{
	...
	if($_SESSION['security_validate'] != true)
	{
		exit(json_encode(array('error' => 1, 'content' => '非法操作', 'url' => 'security.php')));
	}
	...
	$result = $GLOBALS['user']->edit_user(array('username' => $user_name, 'password' => $password));
...
function action_do_email_binding ()
{
	...
	if($result == false)
	{
		exit(json_encode(array('error' => 1, 'content' => '绑定邮箱失败，请重新尝试', 'url' => '')));
	}
	else
	{
		//设置为第二步
...
function action_email_validate ()
{
	...
	if(empty($email))
	{
		show_message('您还未绑定邮箱地址！', array('去绑定邮箱', "返回账户安全中心"), array('security.php?act=email_binding', 'security.php'), 'info');
	}
...
function action_to_mobile_binding ()
{
	...
	if($_SESSION['security_validate'] != true)
	{
		show_message('您还未绑定手机号码！', array('去绑定手机号', "返回账户安全中心"), array('security.php?act=mobile_binding', 'security.php'), 'info');
	}
...
function action_do_payment_password_reset()
{
	...
	if(!empty($surplus_password))
	{
		...
		{
			exit(json_encode(array('error' => 1, 'content' => '设置支付密码失败，请重新尝试', 'url' => '')));
		}
	}
	else
	{
		exit(json_encode(array('error' => 1, 'content' => '支付密码不能为空', 'url' => '')));
	}
}
...
function action_do_payment_password_close()
{
	...
	if($_SESSION['security_validate'] != true)
	{
		show_message('非法操作！', '返回上用户安全中心', 'security.php', 'info');
	}
	...
	else
	{
		//释放变量
		$_SESSION['security_validate'] = false;
		show_message('关闭支付密码失败，请重新尝试', '返回上用户安全中心', 'security.php', 'info');
	}
}
