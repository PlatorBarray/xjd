...
function check_password(password) {
	if (password.indexOf(" ") != -1) {
		...
		$(currentForm).find('#password_notice').html("");
...
function check_confirm_password(confirm_password) {
	...
	if (confirm_password != password) {
		...
		$(currentForm).find('#confirm_password_notice').html("");
		_CHECK_RESULT.ERROR = false;
	}
}
...
function sendEmailCode(emailObj, emailCodeObj, sendButton) {
	checkEmail(emailObj, function(result) {
		if (result) {
			...
			var captchaObj = $("#emailForm").find("#captcha");
			$.post(url, {
				...
				if (result == 'ok') {
					...
					$("#emailForm").find("#captcha_notice").html("");
					$("#emailForm").find("#captcha_notice").css({color: '#093'});
				} else {
					alert(result);
					if(captchaObj.size() > 0){
						$("#emailForm").find("#captcha_img").click();
					}
				}
			}, 'text');
		}
		...
		if (result) {
			...
			$.post(url, {
				...
				if (result == 'ok') {
					// 倒计时
					countdown(sendButton);
					$("#mobileForm").find("#captcha_notice").html("");
					$("#mobileForm").find("#captcha_notice").css({color: '#093'});
				} else {
					alert(result);
					if(captchaObj.size() > 0){
						$("#mobileForm").find("#captcha_img").click();
					}
				}
			}, 'text');
		}
