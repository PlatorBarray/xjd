...
//读取文章
$sql='select *  from '.$GLOBALS['ecs']->table('article').'where cat_id in(4,5,8,9,10,12,13) limit 14';
$article_data=$GLOBALS['db']->GetAll($sql);
foreach($article_data as $k=>$v)
{
    $article_data[$k]['add_time']=date('Y-m-d',$v['add_time']);
}
//print_r(get_index_adimg(5));
$smarty->assign('count_img',1);
$smarty->assign('count_i',array());
$smarty->assign('count_nav',array());
$smarty->assign('count_cate',array(5));
$smarty->assign('selfadd',0);
//所有商品分类
$allcategory=get_categories_tree(0);
//print_r($allcategory);die;
$smarty->assign('allcategory',$allcategory);
$smarty->assign('article_data',$article_data);
//print_r(get_index_adimg(5));die;
