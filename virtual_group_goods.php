...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'price')
{
    ...
    else
    {
        ...
        $shop_price  = get_virtual_price($attr_id);
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'allprice')
{
    ...
    //file_put_contents('./66.txt',var_export($ret_result,true));
    die($json->encode($res));
    
    
}

if($_REQUEST['act'] == 'check_out_virtual'){
    include('includes/cls_json.php');
    $json   = new JSON;
    $_SESSION['flow_type'] = 7;
    $_SESSION['extension_code'] = 'virtual_good';
    die($json->encode(1));
}
/*------------------------------------------------------ */
//-- 商品购买记录ajax处理
...
if (!$smarty->is_cached('virtual_group_goods.dwt', $cache_id))
{
...
/* 代码增加_end   By www.ecshop68.com */
 ...
 WHERE s.supplier_id=".$goods[supplier_id]." AND s.status=1";
			 ...
			 get_dianpu_baseinfo($goods['supplier_id'],$shopuserinfo);
                     ...
                     $opint = $GLOBALS['db']->getOne("SELECT value FROM " .$GLOBALS['ecs']->table('shop_config'). " WHERE code = 'shop_opint'");
                    if(empty($opint)){
                    $city = get_city_name($GLOBALS['db']->getOne("SELECT value FROM " .$GLOBALS['ecs']->table('shop_config'). " WHERE code = 'shop_city'"));
                    $pointJson = file_get_contents('http://api.map.baidu.com/geocoder/v2/?ak=dDbtTxnNUxSPt68hA2PhrNqS&output=json&address='.$GLOBALS['db']->getOne("SELECT value FROM " .$GLOBALS['ecs']->table('shop_config'). " WHERE code = 'shop_address'").'city='.$city);//获取地图坐标
    ...
    /* 获取相同区域下的团购产品 */
    $other_virtual_goods =  get_other_virtual_goods($goods_id);
    $smarty->assign('other_virtual_goods',$other_virtual_goods);
}
...
$smarty->display('virtual_group_goods.dwt',      $cache_id);
...
function get_dianpu_baseinfo($suppid=0,$suppinfo){
    ...
    $smarty->assign('region', get_province_city($_goods_attr['shop_province'],$_goods_attr['shop_city']));
        ...
        $pointJson = file_get_contents('http://api.map.baidu.com/geocoder/v2/?ak=dDbtTxnNUxSPt68hA2PhrNqS&output=json&address='.$_goods_attr['shop_address'].'city='.$city);//获取地图坐标
        $pointArr = json_decode($pointJson,true);
        }else{
         $option = explode(',',$opint);
         $pointArr['result']['location']['lng']=$option[0];
...
function get_virtual_price($attr_id){
    $sql = "select attr_price from " .$GLOBALS['ecs']->table('goods_attr')." where goods_attr_id =".$attr_id;
 ...
 * 获取城市名称
 */
function get_city_name($regoin_id){
    $sql = "select region_name from ".$GLOBALS['ecs']->table('region')." where region_id=".$regoin_id;
    $res = $GLOBALS['db']->getOne($sql);
    return $res;
}

function get_other_virtual_goods($id){
 ...
 $sql = "select g.goods_id,g.goods_name,g.goods_number,g.supplier_id,g.cat_id,g.add_time,g.valid_date,g.last_update,g.shop_price,g.goods_thumb,ifnull(salenum,0) as salenum from ".$GLOBALS['ecs']->table("goods").
            ...
            . " where g.is_delete = '0' and g.is_real='0' and g.extension_code = 'virtual_good' ".$wherecat."  and g.goods_id in (".implode(',',$goods_ids_array).") LIMIT 0,10";
        $res = $GLOBALS['db'] -> getAll($sql);
        return $res;
}
