...
if($is_virtual){
    //直接跳到虚拟商品页面
    ecs_header("Location: virtual_group_goods.php?id=".$goods_id."\n");
    exit;
}
...
/* 代码增加_end   By www.ecshop68.com */
 ...
 WHERE s.supplier_id=".$goods[supplier_id]." AND s.status=1";
        ...
        //收藏个数统计
        $sql = "SELECT COUNT(rec_id) FROM " . $GLOBALS['ecs']->table('collect_goods') ." WHERE goods_id = '$goods_id'";
...
//            $subtotal += $val['rank_price'] * $val['goods_number'];
        ...
        foreach($goods_res as $key => $val)
        {
            if ($val['goods_attr'] != '')
            {
                ...
                foreach ($goods_attr_array as $_attr)
                {
                    $goods_attr_price += $GLOBALS['db']->getOne(
                        'SELECT attr_price FROM ' . $GLOBALS['ecs']->table('goods_attr') . ' WHERE goods_attr_id = ' . $_attr
                    );
                    $goods_attr[] = $_goods_attr[$_attr];
                }
                $goods_res[$key]['goods_attr_price'] = $goods_attr_price;
                $goods_res[$key]['goods_attr_str'] = sprintf($format, implode('，', $goods_attr));
            }
        }
		...
		foreach($goods_res as $key => $val)
		{
            ...
            if(isset($val['goods_attr_price']))
            {
                ...
                $subtotal += $val['rank_price'] + $val['goods_attr_price'];
            }
            else
            {
