    ...
    if ($row = $GLOBALS['db']->getRow($sql))
    {
        ...
        if ($row['user_rank'] == 0)
        {
            ...
            if ($row = $GLOBALS['db']->getRow($sql))
            {
                ...
                $sql="SELECT rank_name from ".$GLOBALS['ecs']->table('user_rank')."where rank_id= ".$row['rank_id'];
                $_SESSION['user_rank_name']       = $GLOBALS['db']->getOne($sql);
...
function assign_ur_here($cat = 0, $str = '')
{
    ...
    $ur_here    = '<a href="." class="index">' . $GLOBALS['_LANG']['home'] . '</a>';
    ...
    if ($filename != 'index')
    {
        ...
        if (in_array($filename, array('category', 'goods', 'article_cat', 'article', 'brand')))
        {
            ...
            elseif ('article_cat' == $filename || 'article' == $filename)
            {
                if ($cat > 0)
                {
                    $cat_arr_article = get_article_parent_cats($cat);
            ...
            if (!empty($cat_arr)){
                ...
                foreach ($cat_arr AS $val){
                    ...
                    $ur_here   .= " <span class='crumbs-arrow'>&gt;</span> <div class='crumbs-nav crumbs-nav".$op."'><a class='crumbs-title' href='" . build_uri($type, $args, $val['cat_name']) .
								"'>" . htmlspecialchars($val['cat_name']) . "<i class='icon'></i></a>";
					$ur_here   .= "<div class='crumbs-drop'><ul class='crumbs-drop-list'>" . get_cat_all($val['parent_id']) . "</ul></div></div>";
					$op++;
                }
            }
			if (!empty($cat_arr_article)){
                krsort($cat_arr_article);
				$op = 0;
                foreach ($cat_arr_article AS $val){
                    $page_title = htmlspecialchars($val['cat_name']) . '_' . $page_title;
                    $args       = array($key => $val['cat_id']);
                    $ur_here   .= " <span class='crumbs-arrow'>&gt;</span><a class='last' href='" . build_uri($type, $args, $val['cat_name']) .
        ...
        else
        {
            ...
            if ('group_buy' == $filename)
            {
                ...
                $args       = array('gbid' => '0');
                $ur_here   .= ' <span class="crumbs-arrow">&gt;</span> <a href="group_buy.php" class="last">' .
            ...
            elseif ('auction' == $filename)
            {
                ...
                $args       = array('auid' => '0');
                $ur_here   .= ' <span class="crumbs-arrow">&gt;</span> <a href="auction.php" class="last">' .
            ...
            elseif ('snatch' == $filename)
            {
                ...
                $args       = array('id' => '0');
                $ur_here   .= ' <span class="crumbs-arrow">&gt;</span><a href="snatch.php" class="last">' .                                 $GLOBALS['_LANG']['snatch_list'] . '</a>';
            }
            /* 批发 */
            elseif ('wholesale' == $filename)
            {
                ...
                $args       = array('wsid' => '0');
                $ur_here   .= ' <span class="crumbs-arrow">&gt;</span> <a href="wholesale.php" class="last">' .
            ...
            elseif ('exchange' == $filename)
            {
                ...
                $args       = array('wsid' => '0');
                $ur_here   .= ' <span class="crumbs-arrow">&gt;</span> <a href="exchange.php" class="last">' .
                                $GLOBALS['_LANG']['exchange'] . '</a>';
            }
            /* 预售 */
            elseif ('pre_sale' == $filename)
            {
                $page_title = $GLOBALS['_LANG']['pre_sale'] . '_' . $page_title;
                $args       = array('wsid' => '0');
                $ur_here   .= ' <span class="crumbs-arrow">&gt;</span> <a href="pre_sale.php" class="last">' .
                                $GLOBALS['_LANG']['pre_sale'] . '</a>';
            }
            /* 虚拟团购 */
            elseif ('virtual_group' == $filename)
            {
                $page_title = $GLOBALS['_LANG']['virtual_group'] . '_' . $page_title;
                $args       = array('wsid' => '0');
                $ur_here   .= ' <span class="crumbs-arrow">&gt;</span> <a href="virtual_group.php" class="last">' .
                                $GLOBALS['_LANG']['virtual_group'] . '</a>';
            }
            /* 其他的在这里补充 */
        }
    }
    ...
    if (!empty($str))
    {
        ...
        $ur_here    .= ' <span class="crumbs-arrow">&gt;</span> <span class="last">' . $str . '</span>';
    }
...
{
    $sql = 'SELECT id, number, type, sort_order, ext_info FROM ' . $GLOBALS['ecs']->table('template') .
        " WHERE filename = '$tmp' AND type > 0 AND remarks ='' AND theme='" . $GLOBALS['_CFG']['template'] . "' order by sort_order asc";
   ...
   foreach ($res AS $row)
    {
        switch ($row['type'])
        {
            case 1:
                ...
                // 排序->楼层
                $GLOBALS['smarty']->assign('goods_cat_' . $row['id'] . '_sort_order', ($row['sort_order'] + 1));
                $ext_info = unserialize($row['ext_info']);
                if(empty($ext_info['cat_color'])){
                	$ext_info['cat_color'] = "#E31939";
                }else if(strpos($ext_info['cat_color'], '#') == false){
                	$ext_info['cat_color'] = '#'.$ext_info['cat_color'];
                }
                // 扩展信息
                $GLOBALS['smarty']->assign('goods_cat_' . $row['id'] . '_ext_info', $ext_info);
            break;
            case 2:
                /* 品牌的商品 */
...
function assign_pager($app, $cat, $record_count, $size, $sort, $order, $page = 1,
                        $keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr='', $url_format='', $sch_array='',$filter=0)
{
	//$shangjian = "category.php?category={$cat}&display={$pager['display']}&brand={$brand}&price_min={$price_min}&price_max={$price_max}&filter_attr={$filter_attr}&page={$pager['page']}&sort=goods_id&order=".($pager['sort'] == 'goods_id' && $pager.order == 'DESC'} -->ASC<!-- {else} -->DESC<!-- {/if} -->#goods_list"
    ...
    switch ($app)
    {
    	case 'supplier':
            ...
            break;
        case 'category':
            $uri_args = array('cid' => $cat, 'bid' => $brand, 'price_min'=>$price_min, 'price_max'=>$price_max,'filter'=>$filter, 'filter_attr'=>$filter_attr, 'sort' => $sort, 'order' => $order, 'display' => $display_type);
            break;
        case 'article_cat':
    ...
    $GLOBALS['smarty']->assign('shangjian', build_uri($app, array('cid'=>$cat,'display'=>$pager['display'],'bid'=>$brand,'price_min'=>$price_min,'price_max'=>$price_max,'filter'=>$filter,'filter_attr'=>$filter_attr,'page'=>$pager['page'],'sort'=>'goods_id','order'=>(($pager['sort'] == 'goods_id' && $pager['order'] == 'DESC') ? 'ASC' : 'DESC'))));
    ...
    if ($row = array_shift($GLOBALS['libs'][$key]))
    {
        ...
        switch($row['type'])
        {
            case 1:
                ...
                $str .= '{assign var="sort_order" value=$goods_cat_' . $row['id'] . '_sort_order}';
                $str .= '{assign var="ext_info" value=$goods_cat_' . $row['id'] . '_ext_info}';
                break;
            case 2:
                // 品牌的商品
...
function recalculate_price()
{
    ...
    foreach ($res AS $row)
    {
        ...
        $goods_price = get_final_price($row['goods_id'], 1, true, $attr_id);
...
function assign_comment($id, $type, $page = 1,$comment_level=0)
{
	...
	if ($comment_level == '1')
	{
		$sql_comment=" AND c.comment_rank in ('4', '5') ";
	}
	elseif ($comment_level == '2')
	{
		$sql_comment=" AND c.comment_rank in ('3', '2') ";
	}
	elseif ($comment_level == '3')
	{
		$sql_comment=" AND c.comment_rank in ('1') ";
	}else{
		$sql_comment=" ";
	}
    ...
    $sql = 'SELECT c.*, u.headimg FROM ' . $GLOBALS['ecs']->table('comment') . " AS c " .
			" LEFT JOIN " . $GLOBALS['ecs']->table('users') . " AS u ON c.user_name = u.user_name " .
            " WHERE c.id_value = '$id' AND c.comment_type = '$type' AND c.status = 1 AND c.parent_id = 0 $sql_comment ".
...
function get_cat_all($p)
{
	$arr = "";
	$cat_all = $GLOBALS['db']->getAll("SELECT cat_id, cat_name FROM " . $GLOBALS['ecs']->table('category') . " WHERE parent_id = " . $p);
	foreach($cat_all AS $val)
	{
		$arr .= "<li><a href='category.php?id=" . $val['cat_id'] . "'>" . $val['cat_name'] . "</a></li>";
	}
	return $arr;
}
...
function get_library_number($library, $template = null)
{
    ...
    else
    {
        ...
        $num = isset($static_page_libs[$template][$lib]) ? $static_page_libs[$template][$lib] :  10;
    }
