 ...
 * ============================================================================
 * $Author: derek $
 * $Id: ucenter.php 17217 2011-01-19 06:29:08Z derek $
 */
    ...
    function login($username, $password, $rember=0)
    {
        ...
        elseif($uid == -1)
        {
        	...
        	if($result){
        		...
        		//如果用户存在则同步到Ucenter中
        		return $this->add_user($username, $password, $email);
        	}
...
//             	$this->db->query('INSERT INTO ' . $GLOBALS['ecs']->table("users") . "(`user_id`, `email`, `user_name`, `password`, `reg_time`, `last_login`, `last_ip`) VALUES ('$uid', '$email', '$username', '$password', '$reg_date', '$reg_date', '$ip')");
    ...
    function edit_user($cfg, $forget_pwd = '0')
    {
        ...
        $set_str = '';
        $valarr =array('email'=>'email', 'gender'=>'sex', 'bday'=>'birthday' , 'password'=>'password', 'mobile_phone'=>'mobile_phone');
    ...
    function remove_user($id)
    {
        ...
        if($info != 0){
        	$uid = $info[0];
        	// 删除UCenter
        	$affected_rows = uc_call('uc_user_delete', $uid);
        }
