    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
            ...
            $str.='<li>
              ...
              <div class="p-comm"><span class="p-price main-color">'.$goods['shop_price'].'</span> </div>
            </li>';
        }
...
$arr[$key]['shop_price'] =  price_format($rows['shop_price']);
			...
			$str .= '<li><div class="item_wrap">
				...
				<div class="dd">
					...
					<div class="btn">
						...
						$goods['goods_type'] . ', \'' .
						$goods['goods_thumb'] . '\', \'' . $showprice . '\')"></a><span class="price main-color"><strong>' . $showprice . '</strong></span>
					</div>
				</div>
			</div></li>';
        }
...
function insert_cart_info()
{
    ...
    $sql = 'SELECT c.*,pa.*,IF(c.extension_code = "package_buy",act_name,g.goods_name) as goods_name, ' .
    ...
    $row = $GLOBALS['db']->GetAll($sql);
        ...
        $arr[$k]['short_name']   = $GLOBALS['_CFG']['goods_name_length'] > 0 ?
				...
				$arr[$k]['url']          = ($v['extension_code'] == 'package_buy' ? 'package.php#floor'.$v['act_id'] : build_uri('goods', array('gid' => $v['goods_id']), $v['goods_name']));
...
{
	$sql = 'SELECT * FROM '. $GLOBALS['ecs']->table('chat_third_customer') . ' WHERE supplier_id = 0 ORDER BY cus_type, cus_name';
	$row = $GLOBALS['db']->GetAll($sql);
	$arr = array();
	foreach($row AS $k=>$v)
	{
...
function insert_supplier_list($arr){
	...
	if(empty($tag_ret)){//没有标签
		$sql = "SELECT ssc.value,ssc.supplier_id,ssc.code FROM ".$GLOBALS['ecs']->table('supplier_street')." as ss,".$GLOBALS['ecs']->table('supplier_shop_config')." as ssc WHERE ss.`status` = 1 AND ssc.supplier_id = ss.supplier_id AND code in('shop_name','shop_logo') order by ss.sort_order limit 48";
