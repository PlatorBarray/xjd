...
//    if (!$exc->is_only('type_name', $id, $val))
    if (!$exc->is_only('type_name', $val, $id))
...
//    }
    ...
    /* 检查类型是否有重复 */
    $sql = "SELECT COUNT(*) FROM " .$ecs->table('bonus_type'). " WHERE supplier_id = 0 AND type_name='$type_name'";
    if ($db->getOne($sql) > 0)
    {
        $link[] = array('text' => $_LANG['go_back'], 'href' => 'javascript:history.back(-1)');
...
//            " WHERE bonus_type_id = '$type_id'";
        " WHERE bonus_type_id = '$type_id' AND is_delete = 0 AND is_on_sale = 1";
    /* 代码修改 By  www.68ecshop.com End */
