...
if ($_REQUEST['act'] == 'replenish')
{
    ...
    {
        $link[] = array('text'=>$_LANG['go_back'],'href'=>'virtual_goods_card.php?act=all_card');
...
elseif ($_REQUEST['act'] == 'edit_replenish')
{
    ...
    $smarty->assign('ur_here',     $_LANG['replenish']);
    $smarty->assign('action_link', array('text'=>$_LANG['go_list'], 'href'=>'virtual_goods_card.php?act=all_card'));
    $smarty->assign('card',        $card);
    $smarty->display('virtual_goods_card_info.htm');
}
        ...
        if($res){
            if($res['is_verification'] == '0'){
                ...
                if($end_date >= $now){
                    ...
                    $sql = "update ".$ecs->table('virtual_goods_card')." set is_verification = 1 , buy_date = ".gmtime()." where card_sn = '$card_sn'";
                    $db -> query($sql);
                    /* 改变订单状态 */
                    ...
                    if($db->getOne($sql)){
                        $sql = "update ".$ecs->table('order_info')." set order_status = '5', shipping_status = '2' , shipping_time_end = ".gmtime()." where order_sn = '$res[order_sn]'";
                    }else{
                        $sql = "update ".$ecs->table('order_info')." set shipping_status = '2' , shipping_time_end = ".gmtime()." where order_sn = '$res[order_sn]'";
                    }
                    $db -> query($sql);
                }else{
                    $result['msg'] = '3';
...
elseif ($_REQUEST['act'] == 'submit_change')
{
    ...
    if (isset($_POST['old_string']) && isset($_POST['new_string']))
    {
        ...
        // 返回
        sys_msg($_LANG['change_key_ok'], 0, array(array('href' => 'virtual_goods_card.php?act=all_card', 'text' => $_LANG['virtual_card_list'])));
    }
}
