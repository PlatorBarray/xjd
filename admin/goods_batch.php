...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if (isset($_POST['checked']))
    {
        ...
        foreach ($_POST['checked'] AS $key => $value)
        {
            ...
            // 查询所有商品货号
            $goods_sns = $db->getAll('SELECT goods_sn FROM ' . $ecs->table('goods'));
       
            foreach($goods_sns as $goods_sn)
            {
                array_push($goods_sns, $goods_sn[goods_sn]);
            }
            if(in_array($field_arr[goods_sn], $goods_sns))
            {
                $errors .= '<br/>商品货号：' . $field_arr[goods_sn] . '&nbsp;&nbsp;商品名称：' . $field_arr[goods_name];
                continue;
            }
            ...
            if (!empty($field_arr['original_img']) || !empty($field_arr['goods_img']) || !empty($field_arr['goods_thumb']))
            {
				...
				//文件批量上传商品时有自定义图片路径的处理方式

				$goods_img = empty($goods_img) ? $field_arr['goods_img'] : $goods_img;
				$goods_thumb = empty($goods_thumb) ? $field_arr['goods_thumb'] : $goods_thumb;
				$original_img = empty($original_img) ? $field_arr['original_img'] : $original_img;

				$goods_gallery['img_url'] = empty($goods_gallery['img_url']) ? $field_arr['goods_img'] : $goods_gallery['img_url'];
				$goods_gallery['img_original'] = empty($goods_gallery['img_original']) ? $field_arr['original_img'] : $goods_gallery['img_original'];
				$goods_gallery['thumb_url'] = empty($goods_gallery['thumb_url']) ? $field_arr['goods_thumb'] : $goods_gallery['thumb_url'];
    ...
    {
    	sys_msg('以下商品上传失败，失败原因：已存在相同货号的商品' . $errors, 0, $link);
    }
    /* 代码修改 By  www.68ecshop.com End */
}
...
{
    $filter = new stdclass;   //代码修改  By   www.68ecshop.com   将 & 删掉
