 ...
 * ============================================================================
 * $Author: langlibin $
 * $Id: user_analysis_order_amount.php 2015-10-27 13:06:08Z langlibin $
 */
...
if ($_REQUEST['act'] == 'list')
{
    ...
    // 查询
    $sql = 'SELECT u.user_name, SUM(oi.goods_amount) order_count FROM '
        . $GLOBALS['ecs']->table('users') . ' u, ' . $GLOBALS['ecs']->table('order_info') . ' oi ' . $where
        . ' AND u.user_id = oi.user_id GROUP BY user_name ORDER BY order_count DESC ' . ' LIMIT 15';
    ...
    /* 显示客服列表页面 */
    assign_query_info();
    $smarty->display('user_analysis_order_amount.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('user_analysis_order_amount.htm'), '',
...
function get_result_list ($start_date, $end_date)
{
    ...
    if($result === false)
    {
        ...
        $sql = 'SELECT u.user_name, SUM(oi.goods_amount) order_count FROM '
            . $GLOBALS['ecs']->table('users') . ' u, ' . $GLOBALS['ecs']->table('order_info') . ' oi ' . $where
            . ' AND u.user_id = oi.user_id GROUP BY user_name ORDER BY order_count DESC '
...
function export ($result)
{
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('C1', '下单金额');
