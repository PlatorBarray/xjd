...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
	...
	$sql1="select * from  " .$ecs->table('goods_activity'). " where goods_id = " .$goods_id;
	$row1=$db->getRow($sql1);
	//echo "<pre>";
	//print_r($row1);
	if($row1 && $_REQUEST['act'] != 'update'){
		sys_msg($_LANG['yjtj_select_goods']);
	}
...
elseif ($_REQUEST['act'] == 'settle_money')
{
    ...
    {
    	$links = array(
    			array('href' => 'auction.php?act=edit&id='.$id,  'text' => '编辑拍卖活动'),
    			array('href' => 'auction.php?act=list', 'text' => $_LANG['back_auction_list'])
    	);
        sys_msg($_LANG['invalid_status'], 0, $links);
    }
    ...
    sys_msg($_LANG['settle_deposit_ok'],0, $links);
}
