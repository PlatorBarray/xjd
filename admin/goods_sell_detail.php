...
if ($_REQUEST['act'] == 'list')
{
    ...
    $result = get_sell_detail($start_date, $end_date, $cat_id, $brand_id, $sel_shop, $supplier_id);
...
elseif ($_REQUEST['act'] == 'export')
{
    ...
    $sql = "SELECT og.goods_name, og.goods_attr, i.order_sn, og.goods_number sells_count, og.goods_price, i.add_time FROM "
        ...
        . ' b ON g.brand_id = b.brand_id ' . $where . ' GROUP BY og.goods_name, i.order_sn, og.goods_price, i.add_time ORDER BY sells_count';
 ...
 * @return array
 */
function get_sell_detail($start_date, $end_date, $cat_id, $brand_id, $sel_shop, $supplier_id)
{
    $result = get_filter();
    if($result === false)
    {
        ...
        elseif($filter['sel_shop'] == 2)
        {
            ...
            {
                $where .= ' AND i.supplier_id = ' . $filter['supplier_id'];
            }
        }
