...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    if(empty($_GET['goods_id']))  $is_acts=1;
        else $is_acts=2;
    $smarty->assign('is_acts',$is_acts);
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral,exclusive, suppliers_id, cost_price,province,city)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral','$exclusive', '$suppliers_id', '$cost_price','$_POST[province]','$_POST[city]')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code,exclusive, rank_integral, cost_price,province,city)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote', '$zhekou', '$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$is_buy','$buymax','$buymax_start_date','$buymax_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code','$exclusive', '$rank_integral', '$cost_price','$_POST[province]','$_POST[city]')";
        }
    }
    else
    {
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['type']))
    {
        ...
        if ($_POST['type'] == 'trash')
        {
            ...
            /* 代码增加 By  www.68ecshop.com Start */
            $sql = 'SELECT bonus_type_id FROM ' . $GLOBALS['ecs']->table('goods') . " WHERE goods_id in ($goods_id)";
            $bonus_type_id =  $GLOBALS['db']->getOne($sql);
            if(!empty($bonus_type_id))
            {
                $sales_pro_list = ' [商品红包] ';
            }
            $sql = 'SELECT is_exchange FROM ' . $GLOBALS['ecs']->table('exchange_goods') . " WHERE goods_id in ($goods_id)";
            $is_exchange =  $GLOBALS['db']->getOne($sql);
            if(!empty($is_exchange))
            {
                $sales_pro_list .= ' [积分商城] ';
            }
            $sql = 'SELECT package_id FROM ' . $GLOBALS['ecs']->table('package_goods') . " WHERE goods_id in ($goods_id)";
            $package_id =  $GLOBALS['db']->getOne($sql);
            if(!empty($package_id))
            {
                $sales_pro_list .= ' [超值礼包] ';
            }
            $sql = 'SELECT act_id FROM ' . $GLOBALS['ecs']->table('goods_activity') . " WHERE goods_id in ($goods_id) AND act_type = 2";
            ...
            else
            {
                ...
                sys_msg('所选商品包含参加促销活动的商品，不能加入回收站。', 0, $link);
            }
            /* 代码增加 By  www.68ecshop.com End */
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    if(!empty($sales_pro_list))
    {
        make_json_error('此商品参加了' . $sales_pro_list . '促销活动，不能放入回收站。');
...
//-- 搜索入驻商
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'search_suppliers')
{
    include_once(ROOT_PATH . 'includes/cls_json.php');
    $json = new JSON;
    ...
    {
        $sql_supplier = "SELECT supplier_id, supplier_name FROM " . $GLOBALS['ecs']
                ->table("supplier") . " WHERE status='1' ORDER BY supplier_id ";
    }
    else
    {
        $sql_supplier = "SELECT supplier_id, supplier_name FROM " . $GLOBALS['ecs']
                ->table("supplier") . " WHERE status='1' AND supplier_name LIKE '%" . trim($filters
                ->keyword) . "%' ORDER BY supplier_id ";
    }
    $arr = $GLOBALS['db']->getAll($sql_supplier);
...
elseif ($_REQUEST['act'] == 'export')
{
    ...
    // 入驻商ID
    if(empty($_REQUEST['suppliers_exists']))
    {
        $where = 'WHERE is_delete = 0 AND is_real = 1 AND supplier_id = 0';
    }
    else
    {
        if($_REQUEST['suppliers_id'] > 0)
        {
            $where = 'WHERE is_delete = 0 AND is_real = 1 AND supplier_id = ' . $_REQUEST['suppliers_id'];
        }
        else
        {
            $where = 'WHERE is_delete = 0 AND is_real = 1 AND supplier_id <> 0';
    ...
    {
        $where .= ' AND ' . get_children($_REQUEST['cat_id']);
    }
    // 品牌
...
function list_link($is_add = true, $extension_code = '', $supp = '')
{
    ...
    if(!empty($supp))
	{
		$href = 'goods.php?act=list&supp=1';
	}
	else
	{
...
//
//			$data = $db->getRow($sql1);
            $user_info = $db->getRow('SELECT * FROM ' . $ecs->table('users') . ' WHERE user_id = ' . $row['user_id']);
            $data = get_rank($user_info[user_rank], $user_info[rank_points]);
            /* 代码修改 By  www.68ecshop.com End */
            $GLOBALS['tongbu_user_discount'] = $data['discount']/100;
			$GLOBALS['tongbu_user_rank'] = $data['rank_id'];
		}else{
 ...
 * @return array
 */
function get_rank($user_rank, $rank_points)
{
    global $db,$ecs;

    $row = $db->getRow(
        'SELECT discount, rank_id, special_rank FROM ' . $ecs->table('user_rank') . " WHERE rank_id = '$user_rank'"
    );
    if (!empty($row))
    {
        if ($row['special_rank'])
        ...
        {
            return $db->getRow(
                'SELECT discount, rank_id FROM ' . $ecs->table('user_rank') . " WHERE min_points > '$rank_points' ORDER BY min_points ASC LIMIT 1"
            );
        }
    }
    else
    {
        return $db->getRow(
            'SELECT discount, rank_id FROM ' . $ecs->table('user_rank') . " WHERE '$rank_points' > min_points ORDER BY min_points DESC LIMIT 1"
        );
    }
}
/* 代码增加 By  www.68ecshop.com End */
