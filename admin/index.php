...
elseif ($_REQUEST['act'] == 'main')
{
    ...
    /*后台管理起始页_修改_START_www.68ecshop.com*/
...
//    " WHERE supplier_id=0  " . order_query_sql('await_pay'));
    ...
    //    $GLOBALS['ecs']->table('users') . " AS u ON ua.user_id = u.user_id WHERE  ua.process_type = '1'  AND ua.is_paid = '0' ";
    ...
    $order['new_repay'] = $db->getOne('SELECT COUNT(*) FROM '.$ecs->table('back_order').' WHERE status_back < 6 AND status_back != 3 AND back_type = 4 AND supplier_id=0');// 代码修改   By  www.68ecshop.com End
    //退货申请
    // 代码修改   By  www.68ecshop.com Start
//    $order['returns'] = $db->getOne('SELECT COUNT(*) FROM '.$ecs->table('back_order').' WHERE status_back=5 AND back_type=1 AND supplier_id=0');
    $order['returns'] = $db->getOne('SELECT COUNT(*) FROM '.$ecs->table('back_order').' WHERE status_back < 6 AND status_back != 3 AND back_type = 1 AND supplier_id=0');
    // 代码修改   By  www.68ecshop.com End
    //缺货登记
    // 代码修改   By  www.68ecshop.com Start
...
//    " WHERE supplier_id=0 " . order_query_sql('finished');
    ...
    /*后台管理起始页_修改_END_www.68ecshop.com*/
