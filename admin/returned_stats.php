 ...
 * ============================================================================
 * $Author: langlibin $
 * $Id: returned_stats.php 2015-10-25 16:00:08Z langlibin $
 */
...
{
    admin_priv('after_sells_stats');
    // 查询条件
    $where = ' WHERE b.add_time >=' . $start_date . ' AND b.add_time <=' . $end_date;
    // 按店铺查询
    if($sel_shop == 1)
    {
        // 平台方
    ...
    $sql = "SELECT FLOOR((b.add_time - $start_date) / (24 * 3600)) date, SUM(b.refund_money_2) refund_money FROM "
        . $ecs->table('back_order') . ' b ' . $where . ' AND b.back_type = 1 GROUP BY date ORDER BY date';
    // 退款金额统计
    ...
    {
        $date_arr[$value['date_arr']] = $value['refund_money'];
    }
    // 取得图表数据
    ...
    $smarty->assign('returned_info', $result['item']);
    ...
    // 走势图纵坐标
    $smarty->assign('refund_money', $refund_money);
    // 入驻商
    ...
    assign_query_info();
    $smarty->display('returned_stats.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('returned_stats.htm'), '',
...
elseif ($_REQUEST['act'] == 'export')
{
    ...
    $result = $db->getAll(
        ...
        . ' AND o.order_id = b.order_id AND b.user_id = u.user_id AND b.back_type = 1 ORDER BY order_sn DESC'
    );
    foreach ($result AS $key => $value)
    {
    ...
    // 设置sheet名
    $objPHPExcel->getActiveSheet()->setTitle('退货统计');
    // 设置表格宽度
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('B1', '退货编号');
    ...
    $objPHPExcel->getActiveSheet()->setCellValue('F1', '退货金额');
    $objPHPExcel->getActiveSheet()->setCellValue('G1', '退货状态');
    ...
    foreach($result as $value)
    {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('F' . $row_num, $value['refund_money_2']);
        $objPHPExcel->getActiveSheet()->setCellValue('G' . $row_num, $value['status_back_val']);
        $row_num++;
    }
    $outputFileName = '退货统计_' . time() . '.xls';
...
function get_result_list($start_date, $end_date, $sel_shop, $supplier_id)
{
    ...
    if($result === false)
    {
        ...
        // 查询条件
        $where = ' WHERE b.add_time >=' . $filter['start_date'] . ' AND b.add_time <=' . $filter['end_date'];
        // 按店铺查询
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('back_order') . ' b ' . $where . ' AND b.back_type = 1';
