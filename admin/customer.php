...
function action_search_users ()
{
	...
	$sql = "SELECt a.user_id, a.user_name FROM " . $GLOBALS['ecs']->table('admin_user') . " AS a WHERE 1=1 " . $where . " AND a.user_id not in (SELECT b.user_id FROM " . $GLOBALS['ecs']->table('chat_customer') . " AS b WHERE a.user_id = b.user_id) ";
...
function customer_info ($cus_id)
{
	...
	$sql = "SELECT c.*, u.user_name " . "FROM " . $GLOBALS['ecs']->table('chat_customer') . " AS c " . "LEFT JOIN " . $GLOBALS['ecs']->table('admin_user') . " AS u ON u.user_id = c.user_id " . "WHERE c.cus_id = '$cus_id' ";
...
function customer_list ()
{
	...
	if($result === false)
	{
		...
		$sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('chat_customer') . " WHERE 1=1 AND supp_id = -1 $where" . " order by " . $filter['sort_by'] . " " . $filter['sort_order'];
		...
		/* 查询 */
		$sql = "SELECT a.*, b.user_name " . "FROM " . $GLOBALS['ecs']->table('chat_customer') . " AS a LEFT JOIN " . $GLOBALS['ecs']->table('admin_user') . " AS b ON (a.user_id = b.user_id) WHERE 1=1 AND supp_id = -1 $where " . " ORDER BY $filter[sort_by] $filter[sort_order] " . " LIMIT " . $filter['start'] . ", $filter[page_size]";
