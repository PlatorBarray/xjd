...
$display2 = array();//array('tc_express');//同城快递
$display3_not = array('pups','tc_express');
/*------------------------------------------------------ */
//-- 配送方式列表
/*------------------------------------------------------ */
...
if ($_REQUEST['act'] == 'list')
{
    ...
    for ($i = 0; $i < count($modules); $i++)
    {
        ...
        /* 检查该插件是否已经安装 */
        $sql = "SELECT shipping_id, shipping_name, shipping_desc, insure, support_cod,shipping_order,is_default_show FROM " .$ecs->table('shipping'). " WHERE shipping_code='" .$modules[$i]['code']. "' and supplier_id=".$_SESSION['supplier_id']." ORDER BY shipping_order";
        $row = $db->GetRow($sql);
...
elseif ($_REQUEST['act'] == 'install')
{
    ...
    /* 检查该配送方式是否已经安装 */
    $sql = "SELECT shipping_id FROM " .$ecs->table('shipping'). " WHERE shipping_code = '$_GET[code]' and supplier_id=".$_SESSION['supplier_id'];
    $id = $db->GetOne($sql);

    if ($id > 0)
    {
        /* 该配送方式已经安装过, 将该配送方式的状态设置为 enable */
        $db->query("UPDATE " .$ecs->table('shipping'). " SET enabled = 1 WHERE shipping_code = '$_GET[code]' and supplier_id=".$_SESSION['supplier_id']." LIMIT 1");
    }
    else
    {
        ...
        $sql = "INSERT INTO " . $ecs->table('shipping') . " (" .
                ...
                ",supplier_id) VALUES (" .
                    ...
                    addslashes($_LANG[$modules[0]['desc']]) . "', '$insure', '" . intval($modules[0]['cod']) . "', 1, '" . addslashes($modules[0]['print_bg']) . "', '" . addslashes($modules[0]['config_lable']) . "', '" . $modules[0]['print_model'] . "', $support_pickup, $is_default_show,".$_SESSION['supplier_id'].")";
        $db->query($sql);
        $id = $db->insert_Id();
    }
...
elseif ($_REQUEST['act'] == 'uninstall')
{
    ...
    /* 获得该配送方式的ID */
    $row = $db->GetRow("SELECT shipping_id,shipping_code, shipping_name, print_bg FROM " .$ecs->table('shipping'). " WHERE shipping_code='$_GET[code]' and supplier_id=".$_SESSION['supplier_id']);
...
elseif ($_REQUEST['act'] == 'is_default_show')
{
	...
	$sql = "select shipping_id,shipping_name from ".$ecs->table("shipping")." where is_default_show=0 and supplier_id=".$_SESSION['supplier_id']." and enabled=1 and shipping_code='".$_REQUEST['code']."'";
	$info = $db->getRow($sql);
	if($info){
		...
		set_default_show($shipping_id,$_SESSION['supplier_id']);
