 ...
 * ============================================================================
 * $Author: derek $
 * $Id: presale.php 17217 2011-01-19 06:29:08Z niqingyang $
 */
define('IN_ECS', true);
...
function action_search_goods()
{
	...
	//入驻商编号
	$filter->supplier_id = $_SESSION['supplier_id'];
	//非虚拟商品
	$filter->is_virtual = 0;
...
function pre_sale_list ()
{
	...
	if($result === false)
	{
		...
		$where = $where.((! empty($filter['keyword'])) ? " AND goods_name LIKE '%" . mysql_like_quote($filter['keyword']) . "%'" : '');
