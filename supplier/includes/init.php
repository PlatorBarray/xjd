    ...
    $_REQUEST['act'] != 'forget_pwd' && $_REQUEST['act'] != 'reset_pwd' && $_REQUEST['act'] != 'check_order')
{
    ...
    if (!empty($_COOKIE['ECSCP']['supplier_id']) && !empty($_COOKIE['ECSCP']['supplier_user_id']) && !empty($_COOKIE['ECSCP']['supplier_pass']))
    {
        // 找到了cookie, 验证cookie信息
        //$sql = 'SELECT u.user_id, u.user_name, u.password,  u.last_login, s.supplier_id ' .
        //        ' FROM ' .$ecs->table('users') . " AS u ,  ". $ecs->table('supplier') ." AS s ".
        //        " WHERE u.user_id =s.user_id and s.supplier_id= '" . intval($_COOKIE['ECSCP']['supplier_id']) . "'";
        //$row = $db->GetRow($sql);
        ...
        {
            // 没有找到这个记录
            setcookie($_COOKIE['ECSCP']['supplier_id'],   '', 1);
            setcookie($_COOKIE['ECSCP']['supplier_pass'], '', 1);
        ...
        {
            // 检查密码是否正确
            if (md5($row['password'] . $_CFG['hash_code']) == $_COOKIE['ECSCP']['supplier_pass'])
            {
            ...
            {
                setcookie($_COOKIE['ECSCP']['supplier_id'],   '', 1);
				setcookie($_COOKIE['ECSCP']['supplier_user_id'],   '', 1);
                setcookie($_COOKIE['ECSCP']['supplier_pass'], '', 1);
