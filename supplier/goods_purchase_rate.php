...
if ($_REQUEST['act'] == 'list')
{
    ...
    $result = get_purchase_rate($start_date, $end_date, $cat_id, $brand_id);
...
elseif ($_REQUEST['act'] == 'export')
{
    ...
    // 查询条件
    $where1 = ' WHERE i.supplier_id = ' . $_SESSION['supplier_id']
        . ' AND i.add_time >=' . $start_date . ' AND i.add_time <=' . $end_date
        . ' AND ((i.pay_id = 6 AND i.shipping_status = 2) OR (i.pay_id <> 6 AND i.pay_status = 2))';

    $where2 = ' WHERE g.is_delete = 0 AND g.is_virtual = 0 AND g.supplier_status = 1 AND g.supplier_id = ' . $_SESSION['supplier_id'];
    // 品牌
    if(!empty($cat_id))
    {
 ...
 * @return array
 */
function get_purchase_rate($start_date, $end_date, $cat_id, $brand_id)
{
    $result = get_filter();
    if($result === false)
    {
