...
/*初始化数据交换对象 */
$exc   = new exchange($ecs->table("supplier_article"), $db, 'article_id', 'title');
//$image = new cls_image();
...
if ($_REQUEST['act'] == 'add')
{
   ...
   // admin_priv('article_manage');
    ...
    $smarty->assign('cat_id', $_SESSION['supplier_id']);
...
if ($_REQUEST['act'] == 'insert')
{
   ...
   // admin_priv('article_manage');
    ...
    $sql = "INSERT INTO ".$ecs->table('supplier_article')."(title, cat_id, article_type, is_open, author, ".
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    /* 取文章数据 */
    $sql = "SELECT * FROM " .$ecs->table('supplier_article'). " WHERE article_id='$_REQUEST[id]'";
...
if ($_REQUEST['act'] =='update')
{
    ...
    $sql = "SELECT file_url FROM " . $ecs->table('supplier_article') . " WHERE article_id = '$_POST[id]'";
...
function get_articleslist()
{
    ...
    if ($result === false)
    {
        ...
        /* 文章总数 */
        $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('supplier_article'). ' AS a '.
               'LEFT JOIN ' .$GLOBALS['ecs']->table('supplier_article_cat'). ' AS ac ON ac.cat_id = a.cat_id '.
               'WHERE a.supplier_id='.$_SESSION['supplier_id'].' ' .$where;
        ...
        $sql = 'SELECT a.* , ac.cat_name '.
               ...
               'LEFT JOIN ' .$GLOBALS['ecs']->table('supplier_article_cat'). ' AS ac ON ac.cat_id = a.cat_id '.
               'WHERE a.supplier_id='.$_SESSION['supplier_id'].' ' .$where. ' ORDER by '.$filter['sort_by'].' '.$filter['sort_order'];
...
/**
 * 创建入驻商的店铺文章分类(一个入驻商只能创建一个分类)
 */
function create_supplier_article_cat(){
	global $db, $ecs;
	
	$sql = "SELECT count(cat_id) FROM " . $ecs->table('supplier_article_cat') ." WHERE cat_id=".$_SESSION['supplier_id'];
	$num = $db->getOne($sql);
	if($num>0){
		return;
	}else{
		$insql = "INSERT INTO ". $ecs->table('supplier_article_cat') ." (`cat_id`, `cat_name`, `cat_type`, `keywords`, `cat_desc`, `sort_order`, `show_in_nav`, `parent_id`) VALUES (".$_SESSION['supplier_id'].", '".$_SESSION['supplier_name']."', 2, '', '入驻商的店铺文章分类', 50, 0, 0);";
		if($db->query($insql) === false){
			$links[] = array('text' => $_LANG['navigator'], 'href' => 'navigator.php?act=list');
        	sys_msg('店铺文章分类创建失败！', 1);
		}
		return;
	}
}
