...
$exc = new exchange($ecs->table("supplier_nav"), $db, 'id', 'name');
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    elseif ($_REQUEST['step'] == 2)
    {
        ...
        $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $ecs->table('supplier_nav') . " WHERE type = '". $item_type ."' and supplier_id=".$_SESSION['supplier_id']);
        ...
        if($item_ifshow == 1 && $item_type == 'middle')
        {
            ...
            if($arr)
            {
                ...
                $sql = "INSERT INTO " . $GLOBALS['ecs']->table('supplier_nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type,supplier_id) VALUES('$item_name','".$arr['type']."','".$arr['id']."','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type',".$_SESSION['supplier_id'].")";
            }
        }

        if(empty($sql))
        {
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('supplier_nav') . " (name,ifshow,vieworder,opennew,url,type,supplier_id) VALUES('$item_name','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type',".$_SESSION['supplier_id'].")";
        }
        $db->query($sql);
        clear_cache_files();
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    if (empty($_REQUEST['step']))
    {
        ...
        $row = $db->getRow("SELECT * FROM " . $GLOBALS['ecs']->table('supplier_nav') . " WHERE id='$id'");
    ...
    elseif ($_REQUEST['step'] == 2)
    {
        ...
        $row = $db->getRow("SELECT ctype,cid,ifshow,type FROM " . $GLOBALS['ecs']->table('supplier_nav') . " WHERE id = '$id'");
        ...
        if($arr)
        {
            ...
            $sql = "UPDATE " . $GLOBALS['ecs']->table('supplier_nav') .
                " SET name='$item_name',ctype='" . $arr['type'] . "',cid='" . $arr['id'] . "',ifshow='$item_ifshow',vieworder='$item_vieworder',opennew='$item_opennew',url='$item_url',type='$item_type' WHERE id='$id' AND supplier_id=".$_SESSION['supplier_id'];
        }
        else
        {
            //目标不是分类
            ...
            $sql = "UPDATE " . $GLOBALS['ecs']->table('supplier_nav') .
                " SET name='$item_name',ctype='',cid='',ifshow='$item_ifshow',vieworder='$item_vieworder',opennew='$item_opennew',url='$item_url',type='$item_type' WHERE id='$id' AND supplier_id=".$_SESSION['supplier_id'];
        }
...
{
    $id = (int)$_GET['id'];
    $row = $db->getRow("SELECT ctype,cid,type FROM " . $GLOBALS['ecs']->table('supplier_nav') . " WHERE id = '$id' AND supplier_id=".$_SESSION['supplier_id']." LIMIT 1");
    ...
    $sql = " DELETE FROM " . $GLOBALS['ecs']->table('supplier_nav') . " WHERE id='$id' AND supplier_id=".$_SESSION['supplier_id']." LIMIT 1";
    $db->query($sql);
    clear_cache_files();
...
if ($_REQUEST['act'] == 'toggle_ifshow')
{
    ...
    $row = $db->getRow("SELECT type,ctype,cid FROM " . $GLOBALS['ecs']->table('supplier_nav') . " WHERE id = '$id' LIMIT 1");
...
function get_nav()
{
    ...
    if($result === false)
    {
        ...
        $sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('supplier_nav'). " WHERE supplier_id=".$_SESSION['supplier_id'];
        ...
        $sql = "SELECT id, name, ifshow, vieworder, opennew, url, type".
               " FROM ".$GLOBALS['ecs']->table('supplier_nav')." where supplier_id=".$_SESSION['supplier_id'].
...
function nav_update($id, $args)
{
    ...
    return $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('supplier_nav'), $args, 'update', "id='$id'");
}
...
function is_show_in_nav($type, $id)
{
    ...
    return $GLOBALS['db']->getOne("SELECT show_in_nav FROM $tablename WHERE cat_id = '$id' AND supplier_id=".$_SESSION['supplier_id']);
}
