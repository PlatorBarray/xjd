...
if ($_REQUEST['act'] == 'list')
{
    ...
    {
        export($start_date, $end_date, $_LANG);
    }
    ...
    $order_money = $db->getOne(
        'SELECT SUM(goods_amount) FROM ' . $ecs->table('order_info') . ' o ' . $where
    );
    ...
    $order_money_list = $db->getAll(
        "SELECT FLOOR((o.add_time - $start_date) / (24 * 3600)) date, SUM(o.goods_amount) goods_amount FROM "
...
//-- 批量导出数据
function export($start_date, $end_date, $_LANG)
{
    admin_priv('sell_stats');
    ...
    $result = $GLOBALS['db']->getAll(
        "SELECT o.order_sn, u.user_name, o.add_time, o.goods_amount, o.order_status FROM "
...
function get_result_list($start_date, $end_date)
{
    ...
    if($result === false)
    {
        ...
        // 查询条件
        $where = ' WHERE o.supplier_id = ' . $_SESSION['supplier_id']
            . ' AND o.add_time >=' . $filter['start_date'] . ' AND o.add_time <=' . $filter['end_date'];
        // 订单状态
        if($filter['status'] >= 0)
        {
        ...
        /* 查询 */
        $sql = "SELECT o.order_sn, u.user_name, o.add_time, o.goods_amount, o.order_status FROM "
            . $GLOBALS['ecs']->table('order_info') . ' o, ' . $GLOBALS['ecs']->table('users') . ' u '
            . $where . ' AND o.user_id = u.user_id ORDER BY order_sn DESC'
