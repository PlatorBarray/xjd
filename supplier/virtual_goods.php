 ...
 * ============================================================================
 * $Author: sunlizhi $
 * $Id: virtual_goods.php 17217 2015-07-19 06:29:08Z sunlizhi $
*/
define('IN_ECS', true);
...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    ...
    $handler_list = array();
    $handler_list['virtual_card'][] = array('url'=>'virtual_goods_card.php?act=card', 'title'=>$_LANG['card'], 'img'=>'icon_send_bonus.gif');
	...
	{
		$ur_here = $_LANG['01_virtual_goods_list'];
	}
	else
	{
  ...
  //  $smarty->assign('cat_list',     cat_list(0, $cat_id));
    ...
    /* 显示商品列表页面 */
    assign_query_info();
    $htm_file = ($_REQUEST['act'] == 'list') ?('virtual_goods_list.htm')
       : (($_REQUEST['act'] == 'trash') ? 'goods_trash.htm' : 'virtual_goods_list.htm');
...
/*	优化了分类，去掉了之前开发的分类
	...
	$smarty->assign('catstr',$catstr);
        ...
        $sql = "select d.district_id,v.district_name from ".$GLOBALS['ecs']->table('virtual_district')." as d 
            ...
            where d.goods_id = '{$_REQUEST['goods_id']}' and d.supplier_id=".$_SESSION['supplier_id'];
   ...
   create_html_editor('goods_desc', htmlspecialchars($goods['goods_desc'])); /* 修改 by www.68ecshop.com 百度编辑器 */
    ...
    /* 模板赋值 */
    $action_link_supplier = $is_add ? array('href' => 'virtual_goods.php?act=list&extension_code=virtual_good' , 'text' => '返回商品列表'): array('href' => 'virtual_goods.php?act=list&extension_code=virtual_good&supplier_status='.$_REQUEST['supplier_status'] , 'text' => '返回商品列表');
    $smarty->assign('code',    $code);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    { 
                ...
                $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral, supplier_id,supplier_status,valid_date,is_virtual,is_real,extension_code)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$supplier_id', '0','$valid_date','1','0','$code')";
   ...
   $sql = "delete from ".$ecs->table('virtual_district')." where goods_id = '$goods_id' and supplier_id = '$_SESSION[supplier_id]'";
   ...
   foreach($district_list_ids_arr as $k=>$v){
        $sql = "INSERT INTO " . $ecs->table('virtual_district') . " (district_id, goods_id, supplier_id)" .
                "VALUES ('$v','$goods_id','$_SESSION[supplier_id]')";
    $db->query($sql);
   } 
    ...
    {
        $link[0] = array('href' => 'virtual_goods.php?act=product_list&supplier_status='. $_REQUEST['supplier_status'] .'&goods_id=' . $goods_id, 'text' => $_LANG['product']);
    }
    if ($code == 'virtual_card')
    {
        $link[1] = array('href' => 'virtual_goods_card.php?act=replenish&goods_id=' . $goods_id, 'text' => $_LANG['add_replenish']);
    }
    if ($is_insert)
    {
	...
	if($is_insert)
	 {
		$link[3] = array('href' => 'virtual_goods.php?act=list&extension_code=virtual_good' , 'text' => '返回商品列表');
	 }
	 else
	{
		$link[3] = array('href' => 'virtual_goods.php?act=list&extension_code=virtual_good&supplier_status=', 'text' => '返回商品列表');
	 }
...
{   
    ...
    {
        $link[] = array('href' => 'virtual_goods.php?act=trash', 'text' => $_LANG['11_goods_trash']);
    }
    else
    {	
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    if ($exc->edit("is_delete = 1", $goods_id))
    {
        ...
        $url = 'virtual_goods.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'restore_goods')
{
    ...
    $url = 'virtual_goods.php?act=query&' . str_replace('act=restore_goods', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'drop_goods')
{
    ...
    clear_cache_files();
    $url = 'virtual_goods.php?act=query&' . str_replace('act=drop_goods', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'product_list')
{
    ...
    {
        $link[] = array('href' => 'virtual_goods.php?act=list&extension_code=virtual_good', 'text' => $_LANG['cannot_found_goods']);
    ...
    $smarty->assign('action_link',  array('href' => 'virtual_goods.php?act=list&extension_code=virtual_good&supplier_status='.$_REQUEST['supplier_status'], 'text' => $_LANG['01_goods_list']));
...
elseif ($_REQUEST['act'] == 'product_query')
{
    ...
    $smarty->assign('ur_here', $_LANG['18_product_list']);
    $smarty->assign('action_link', array('href' => 'virtual_goods.php?act=list&extension_code=virtual_good', 'text' => $_LANG['01_goods_list']));
    $smarty->assign('product_list',  $product['product']);
    $smarty->assign('use_storage',  empty($_CFG['use_storage']) ? 0 : 1);
    $smarty->assign('goods_id',    $goods_id);
    $smarty->assign('filter',       $product['filter']);
...
elseif ($_REQUEST['act'] == 'product_remove')
{
    ...
    if ($result)
    {
        ...
        $url = 'virtual_goods.php?act=product_query&' . str_replace('act=product_remove', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'product_add_execute')
{
    ...
    if ($insert)
    {
         $link[] = array('href' => 'virtual_goods.php?act=add', 'text' => $_LANG['03_goods_add']);
         $link[] = array('href' => 'virtual_goods.php?act=list&supplier_status='. $_REQUEST['supplier_status'], 'text' => $_LANG['01_goods_list']);
         $link[] = array('href' => 'virtual_goods.php?act=product_list&supplier_status='. $_REQUEST['supplier_status'] .'&goods_id=' . $product['goods_id'], 'text' => $_LANG['18_product_list']);
    }
    else
    {
         $link[] = array('href' => 'virtual_goods.php?act=list&supplier_status='. $_REQUEST['supplier_status'].'&uselastfilter=1', 'text' => $_LANG['01_goods_list']);
         $link[] = array('href' => 'virtual_goods.php?act=edit&goods_id=' . $product['goods_id'], 'text' => $_LANG['edit_goods']);
         $link[] = array('href' => 'virtual_goods.php?act=product_list&supplier_status='. $_REQUEST['supplier_status'].'&goods_id=' . $product['goods_id'], 'text' => $_LANG['18_product_list']);
    }
...
{
    /* 定义返回 */
    $link[] = array('href' => 'virtual_goods.php?act=product_list&goods_id=' . $_POST['goods_id'], 'text' => $_LANG['item_list']);
...
{	
    if($supplier_status != '')
	{
		$href = 'virtual_goods.php?act=list&supplier_status='.$supplier_status;
	}
	else
	{
		$href = 'virtual_goods.php?act=list';
	}
