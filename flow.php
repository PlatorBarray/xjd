		...
		if ($_SESSION['user_id'] == 0 ){
			...
			if ( $num_total > intval($row_xg['buymax']) )
			{
				...
				if ($_COOKIE['one_step_buy'] == 1){
					...
					$result['message'] .= "您只能再添加 ". $num_else ." 件";
				}
...
elseif ($_REQUEST['step'] == 'change_surplus')
{
    ...
    else
    {
        ...
        else
        {
            ...
            /* 代码修改 By  www.68ecshop.com #1305 Start */
//            if($total['amount_formated'] <= 0){
            if($total['amount'] <= 0)
            {
            /* 代码修改 By  www.68ecshop.com #1305 End */
    ...
    /* 代码修改 By  www.68ecshop.com #1854 Start */
    $result['surplus'] = round($result['surplus'] ,2);
    /* 代码修改 By  www.68ecshop.com #1854 End */
    $json = new JSON();
    die($json->encode($result));
}
    ...
    foreach($order_info as $ok=>$order){
	    ...
	    if ($order['user_id'] > 0 && $order['surplus'] > 0)
	    {
			...
			if($_CFG['sms_user_money_change'] == 1)
			{
				...
				$content = sprintf($_CFG['sms_use_balance_reduce_tpl'],date("Y-m-d H:i:s",time()),$order['surplus'],$users['user_money'],$_CFG['sms_sign']);
	...
	if($favourable_list){
		...
		foreach($favourable_list as $key => $val){
			if(isset($cart_goods['goods_list'][$val['supplier_id']]) ){
				foreach($cart_goods['goods_list'][$val['supplier_id']]['goods_list'] as $k => $v){
					...
					switch($val['act_range'])
					{
						...
						case 1:
							/* 找出分类id的子分类id */
							$id_list = array();
							$raw_id_list = explode(',', $val['act_range_ext']);
							foreach ($raw_id_list as $id)
							{
								$id_list = array_merge($id_list, array_keys(cat_list($id, 0, false)));
							}
							$ids =  array_unique($id_list);						
							...
							break;
						case 2:
						    $brand_ids = explode(',', $val['act_range_ext']);
							if(in_array($v['brand_id'],$brand_ids)){
							$cart_goods['goods_list'][$val['supplier_id']]['favourable'][] = $val;
							}
							break;
						case 3:
							$goods_ids = explode(',', $val['act_range_ext']);
